...
def shortest_path_example(mode):
    ...
    New Code
    '''
    start_loc = env.episodes[0].start_position
    delta = 5
    goal_loc = start_loc.copy()
    for i in range(len(goal_loc)):
        goal_loc[i] = goal_loc[i] + delta * (random.random() - 0.5)
    env.episodes[0].goals[0] = NavigationGoal(position= goal_loc, radius=None)
    '''
    New Code End
    '''
