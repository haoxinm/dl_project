...
class DDPPOTamerTrainer(DDPPOTrainer):
    ...
    def _collect_rollout_step(
        ...
        for (obs, act) in zip(observations, actions):
            estimate_args.append({'observation':obs})
            update_args.append({'observation':obs, 'action':act})

        # with torch.no_grad():
        # self.envs.call(["update_slam"] * self.envs.num_envs, update_args)
        # estimated_goals = self.envs.call(["get_estimate_distance"] * self.envs.num_envs, estimate_args)
        # '''
        # cost = time.time() - t
        # print(cost)
        for i in range(self.envs.num_envs):
            # if dones[i]:
            #     self.gps[i][0] = -1.
            # error = np.abs(estimated_goals[i]/observations[i]['pointgoal_with_gps_compass'] - 1)
            # if np.amax(error) > 0.1:
            #     print(error)
            #     print(estimated_goals[i])
            #     print(observations[i]['pointgoal_with_gps_compass'])
            # observations[i][self.config.TASK_CONFIG.TASK.GOAL_SENSOR_UUID] = estimated_goals[i]
    ...
    def train(self) -> None:
        ...
        # self.envs.call(["init_slam"] * self.envs.num_envs, [{"device": device}] * self.envs.num_envs)
