...
class ORBSLAM2Agent(RandomAgent):
    ...
    def estimate_distance_to_goal(self, observations):
        ...
        estimate_angle = self.initial_goal_position[1] - self_angle
        ...
        return [new_target_distance_val, estimate_angle.numpy() - 0]
    ...
    def update_accumate_position(self, action):
        if action == HabitatSimActions.MOVE_FORWARD: # Move Forward
            ...
            self.accumlate_position = self.accumlate_position 
        else:
            pass
    ...
    def act(self, habitat_observation, random_prob=0.1):
        ...
        if self.set_initial == False and self.initial_goal_position == None :
            self.initial_goal_position = habitat_observation['pointgoal_with_gps_compass']
            self.set_initial = True
        if self.last_position == None:
            self.last_position == torch.tensor([0, 0])
        if self.last_angle == None:
            self.last_angle == torch.tensor(0)
        if self.accumlate_position == None:
            self.accumlate_position == torch.tensor([0, 0])
        if self.accumlate_angle == None:
            self.accumlate_angle == torch.tensor(0)

        print('Estimate Goal', self.estimate_distance_to_goal(habitat_observation))
        print(habitat_observation['pointgoal_with_gps_compass'])
        print('Distance Error:', self.estimate_distance_to_goal(habitat_observation)[0] - habitat_observation['pointgoal_with_gps_compass'][0])
        print('Angle Error', self.estimate_distance_to_goal(habitat_observation)[1] - habitat_observation['pointgoal_with_gps_compass'][1])
...
def main():
    ...
    if args.agent_type == "orbslam2-rgbd":
        print(config.ORBSLAM2)
